...
impl SmsTransport {
    ...
    fn process_incoming_segment(&self, msg: SmsMessage) -> CoreResult<Option<TransportEvent>> {
        ...
        if !self.phone_peers.read().contains_key(&msg.phone) {
            return Err(CoreError::Transport(format!(
                "收到未注册电话号码 {} 的消息，拒绝处理",
                msg.phone
            )));
        ...
        let now = Instant::now();
        reasm.retain(|_, entry| now.duration_since(entry.last_active) < REASSEMBLY_TIMEOUT);
    ...
    fn decode_payload(
        ...
        let peer_id = self.phone_peers.read().get(phone).cloned().ok_or_else(|| {
            CoreError::Transport(format!("收到未注册电话号码 {phone} 的消息，拒绝解析"))
        })?;
