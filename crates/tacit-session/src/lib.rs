...
pub struct SyncSession {
    ...
    seq: AtomicU32,
    /// 上次 sync_log GC 时间，用于限制 GC 频率。
    last_gc: Mutex<Option<Instant>>,
    /// drive_outbound 中提取的 EmitEvent，等待 drain_events 取走。
    pending_events: Mutex<Vec<tacit_sync::SyncAction>>,
}
...
impl SyncSession {
    ...
    /// 提取引擎产生的所有 `EmitEvent` 动作（非事件动作保留在队列中）。
    ///
    /// 应在 `drive_outbound` 之前调用，以确保 `PeerIntroduced`、`PeerRevoked`、
    /// `PeerKeyRotated`、`SyncStarted`、`SyncCompleted` 等事件不被丢弃。
    ///
    /// `handle_transport_event` 可能在调用方 drain 与 drive 之间触发新的 EmitEvent，
    /// 因此最佳实践是每次 `drive_outbound` 前都调用一次此方法。
    pub fn drain_events(&self) -> Vec<tacit_sync::SyncAction> {
        let mut events = self.engine.drain_events();
        // 也提取 drive_outbound 中缓冲的 pending_events
    ...
    /// - `EmitEvent` → 记录 warn 日志（调用方应在调用前先调 `drain_events()` 提取事件）
    ///
    /// 发送失败仅记 warn 日志，不中断后续动作——单条发送失败不应阻塞整个 drain 循环。
    pub async fn drive_outbound(&self) -> CoreResult<()> {
        // 处理待重试的 PendingBlockFetch：当 Data 帧到达但 block 不存在时，
        // 会排队等待 MetaDoc 同步后自动拉取。process_pending 将到期的
        // fetch 转为 RequestDelta 动作入队。
        self.engine.process_pending(std::time::Instant::now())?;
        ...
        for action in actions {
            match action {
                SyncAction::SendData {
                    ...
                    let payload = match codec::encode_payload(block_id.as_ref(), &bytes) {
                        ...
                        Err(e) => {
                            ...
                            // 不标记 entry_id 为已投递——编码失败不代表消息已送达。
                            // 标记为 delivered 会导致永久数据丢失（消息永不会重试）。
                            continue;
                        }
                    };
                    let frame = DataFrame {
                ...
                SyncAction::EmitEvent(event) => {
                    // 将事件存入 pending_events，等 drain_events() 取走。
                    // 这样即使调用方未在 drive_outbound 前调 drain_events()，
                    // 事件也不会丢失——只是延迟到下一次 drain_events() 调用。
                    tracing::trace!(event = ?event, "EmitEvent 存入 pending_events");
                    self.pending_events
                        .lock()
                        .unwrap()
                        .push(tacit_sync::SyncAction::EmitEvent(event));
                }
            }
        }
        // 批量标记 delivered + acknowledged：将 N 次 fsync 合并为 1 次。
        // 同时标记 acknowledged 是因为当前架构无对端 ack 协议——transport Ok
        // 即视为投递完成（at-most-once 合约）。CRDT import 幂等保证安全：
        // 即使帧丢失，run_push_pull 的 RequestDelta pull 仍可恢复数据。
        // 若未来引入 ack 协议，可将 acknowledged 延迟到收到 ack 后标记。
        if !delivered_ids.is_empty() {
            ...
            {
                tracing::warn!(error = %e, "批量标记 delivered+acknowledged 失败");
            }
        }
        // 定期 GC：清理已确认超过 1 小时的 sync_log 记录，防止表无限增长。
        // 用时间门控（5 分钟一次）避免每次 drain 循环都执行 DELETE。
        // drive_outbound 可能以 ~50ms 频率调用，无条件 DELETE 是热路径上的 DB 写。
        {
            let now = Instant::now();
            let mut last_gc = self.last_gc.lock().unwrap();
            let should_gc = last_gc.is_none_or(|t| now.duration_since(t).as_secs() >= 300);
            if should_gc {
                if let Err(e) = self.engine.store_forward().cleanup_acknowledged(3600) {
                    tracing::debug!(error = %e, "sync_log GC 失败（非致命）");
                }
                *last_gc = Some(now);
            }
        }
    ...
    pub fn handle_transport_event(&self, event: TransportEvent) -> CoreResult<()> {
        match event {
            TransportEvent::PeerOnline { peer_id } => {
                ...
                // 信任验证：未信任 peer 不触发 sync，防止本地 deltas 泄漏。
                // 信任状态由对端发送 Capabilities → on_peer_summary 建立。
                // 流程：PeerOnline（此时可能未信任）→ Capabilities 到达 →
                // on_peer_summary 建立信任 → request_sync 触发 push/pull。
                // 对已信任 peer（如重连），PeerOnline 直接 request_sync。
                if self.engine.is_peer_trusted(&peer_id) {
                    // 标记 peer online，使 push_local_change 路由到 live send。
                    // session 路径之前缺少此调用，导致重连后本地编辑仍走
                    // store-and-forward（与 FFI 路径行为不一致）。
                    self.engine.mark_peer_online(&peer_id);
                    self.engine.request_sync(peer_id, SyncReason::PeerOnline)?;
                } else {
                    tracing::debug!(peer = %peer_id, "peer 上线但未信任，等待 Capabilities 后触发 sync");
                }
            }
            TransportEvent::PeerOffline { peer_id } => {
                tracing::info!(peer = %peer_id, "peer 离线");
                // 标记 peer 为离线，使 push_local_change 正确写入
                // store-and-forward 而非直接 SendData（避免数据丢失）。
                self.engine.mark_peer_offline(&peer_id);
            }
    ...
    fn handle_inbound_control(&self, peer_id: &PeerId, msg: ControlMsg) -> CoreResult<()> {
        ...
        // 只有 Capabilities 豁免信任检查——它是新 Trusted peer 首次进入
        // peer_states 的入口（on_peer_summary 内部会查 DB 验证信任状态）。
        // Introduce/KeyRotate 不豁免：合法发送方必然已通过 Capabilities 握手
        // 载入内存 peer_states，豁免它们会让未信任 peer 触发 DB 查询导致 DoS。
        // DoS 防护由 engine 层的 untrusted_cache 负责缓存非 Trusted 的 peer ID。
        let requires_trust = !matches!(msg, ControlMsg::Capabilities(_));
        ...
        let res = match msg {
            ...
            ControlMsg::Capabilities(ann) => {
                ...
                // Capabilities 到达后 peer 已 trusted+online，触发 push/pull。
                // 与 FFI 路径 (ffi_on_peer_summary) 一致：on_peer_summary 后必须 request_sync。
                // 否则 PeerOnline 在 trust 建立前到达会被跳过，Capabilities 后无后续触发。
                self.engine
                    .request_sync(peer_id.clone(), SyncReason::PeerOnline)
            }
