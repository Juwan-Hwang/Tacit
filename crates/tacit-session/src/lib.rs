    ...
    pub async fn drive_outbound(&self) -> CoreResult<()> {
        ...
        for action in actions {
            match action {
                ...
                SyncAction::EmitEvent(_event) => {
                    // drain_actions 会消费所有动作包括 EmitEvent。
                    // 不重新入队——避免 drive_outbound 重复调用时队列无限增长。
                    // FFI 层应在调用 drive_outbound 之前先调用
                    // flush_actions_to_events() 提取并分发 EmitEvent 到 EventBus。
                }
            }
        }
