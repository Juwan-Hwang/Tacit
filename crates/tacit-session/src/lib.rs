...
impl SyncSession {
    ...
    pub async fn drive_outbound(&self) -> CoreResult<()> {
        // 先提取 EmitEvent 动作（drain_events 会将非事件动作重新入队），
        // 避免 drain_actions 消费 EmitEvent 后丢失。
        // FFI 层的 flush_actions_to_events() 也会调用 drain_events，
        // 但这里提前提取确保 session 层不丢事件。
        let events = self.engine.drain_events();
        for action in events {
            if let SyncAction::EmitEvent(event) = action {
                tracing::debug!(event = ?event, "引擎事件");
            }
        }
        ...
        for action in actions {
            match action {
                ...
                SyncAction::EmitEvent(event) => {
                    // EmitEvent 已在 drive_outbound 开头通过 drain_events() 提取，
                    // 到此处说明是 drain_events 和 drain_actions 之间新入队的事件。
                    tracing::debug!(event = ?event, "引擎事件（延迟提取）");
                }
            }
        }
        ...
        match event {
            TransportEvent::PeerOnline { peer_id } => {
                ...
                if self.engine.is_peer_trusted(&peer_id) {
                    ...
                    tracing::debug!(peer = %peer_id, "peer 上线但未信任，跳过 sync");
                }
            }
