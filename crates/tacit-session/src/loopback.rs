...
impl LoopbackTransport {
    ...
    pub fn drain_events(&self) -> Vec<TransportEvent> {
        ...
        while let Ok(msg) = self.rx.try_recv() {
            let event = match msg {
                LoopbackMsg::Data(frame, sender_peer_id) => {
                    // 始终使用通道属性 sender_peer_id 路由 Data 事件，
                    // 与 Control 事件一致——防止 frame.actor_id 被伪造。
                    TransportEvent::Data {
                ...
                LoopbackMsg::Control(msg, sender_peer_id) => {
                    // 始终使用通道属性 sender_peer_id 路由控制消息，
                    // 让 handle_inbound_control 做 claim-vs-sender 验证。
                    // 之前用 extract_peer_id 从消息体提取 peer_id，导致
                    // handle_inbound_control 中的 spoof 检查（m.peer_id != *peer_id）
                    // 永远为 false，形同虚设。
...
impl SyncTransport for LoopbackTransport {
    async fn send_data(
        ...
        self.tx
            .send(LoopbackMsg::Data(frame, self.local_peer_id.clone()))
