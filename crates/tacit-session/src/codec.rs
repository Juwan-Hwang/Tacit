...
///
/// 返回 `CoreResult<Vec<u8>>`，若 block_id 超过 65535 字节则返回错误而非 panic。
pub fn encode_payload(block_id: Option<&BlockId>, delta: &[u8]) -> tacit_core::CoreResult<Vec<u8>> {
    match block_id {
        None => {
        ...
        Some(bid) => {
            ...
            let len = u16::try_from(bid_str.len()).map_err(|_| {
                tacit_core::CoreError::Internal(format!(
                    "block_id 长度 {} 超过 65535 字节上限",
                    bid_str.len()
                ))
            })?;
...
mod tests {
    ...
    fn meta_delta_roundtrip() {
        ...
        let encoded = encode_payload(None, delta).unwrap();
    ...
    fn block_delta_roundtrip() {
        ...
        let encoded = encode_payload(Some(&block_id), delta).unwrap();
        let (bid, decoded) = decode_payload(&encoded).unwrap();
    ...
    fn empty_delta_roundtrip() {
        ...
        let encoded = encode_payload(Some(&block_id), b"").unwrap();
        let (bid, decoded) = decode_payload(&encoded).unwrap();
