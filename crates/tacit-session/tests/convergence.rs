...
/// 注册一个 trusted peer。
fn register_trusted_peer(doc_store: &DocStore, peer_id: &PeerId) {
    let conn = doc_store.store().conn();
    let record = tacit_core::PeerRecord {
        peer_id: peer_id.clone(),
        device_pubkey: format!("{}{}", "a", "").repeat(32),
    ...
    tacit_store::dao::upsert_peer(&conn, &record).unwrap();
}
...
fn two_engines_converge_via_loopback() {
    let (ta, tb) = LoopbackTransport::pair_with_ids(pid(1), pid(2));
    ...
    // 互相注册为 trusted peer（信任验证要求）
    register_trusted_peer(&ds_a, &pid(2));
    register_trusted_peer(&ds_b, &pid(1));

    // 让 engine 内存 peer_states 感知到对方（engine 构造时预加载，之后新增需触发）
    eng_a
        .on_peer_summary(
            pid(2),
    ...
    // B 也创建空文档和空 block（接收方需预存在 block，不盲目创建）
    ds_b.create_doc(doc_id.clone(), "note").unwrap();
    ds_b.create_block(&doc_id, block_id.clone(), BlockKind::Text)
        .unwrap();
...
fn session_codec_roundtrip() {
    ...
    let encoded = tacit_session::encode_payload(None, meta).unwrap();
    ...
    let encoded = tacit_session::encode_payload(Some(&block_id), delta).unwrap();
    let (bid, decoded) = tacit_session::decode_payload(&encoded).unwrap();
