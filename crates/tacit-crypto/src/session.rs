...
mod tests {
    ...
    fn auto_rekey_triggers_at_threshold() {
        ...
        // 第 4 条消息：s1 encrypt_count=3 → 触发 outgoing rekey，计数器重置后本次加密计入 1
        let ct = s1.encrypt(b"trigger-rekey").unwrap();
        // s2 decrypt_count=3 → 触发 incoming rekey，计数器重置后本次解密计入 1
    ...
    fn independent_directional_rekey_no_asymmetric_failure() {
        // 验证非对称消息速率下不会解密失败：
        // s1 发 3 条（触发 s1 outgoing rekey），s2 未解密（decrypt_count=0 不 rekey），
        ...
        // s1 解密：encrypt_count=3 触发 outgoing rekey（仅轮换发送密钥），
        // 但 incoming 密钥未变，所以能正确解密 s2 的消息
