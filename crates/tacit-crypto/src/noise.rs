...
impl NonceCache {
    ...
    fn verify_replay(&self, payload: &[u8]) -> CoreResult<()> {
        ...
        // #13 + #15: 时钟回拨检测、prune、nonce 唯一性检查——全部在单一锁区间内完成。
        {
            ...
            // 硬上限检查——缓存满时强制驱逐最旧条目，防止 DoS
            //
            // 安全权衡说明：
            // - 原策略（拒绝新条目）：攻击者可通过 flood 100K dummy 握手填满缓存，
            //   阻断所有合法新连接——这是极易利用的 DoS。
            // - 当前策略（强制驱逐）：缓存满时按时间戳排序，驱逐最旧（最接近过期）的条目，
            //   为新条目腾出空间。攻击者无法通过填充旧条目来阻断新连接。
            // - 驱逐的旧条目本身已接近过期窗口边缘（60s），驱逐它们的安全影响最小——
            //   它们很快就会自然过期，不再提供重放保护价值。
            // - 内存上限仍由 MAX_NONCE_CACHE_ENTRIES 限制，不会无限增长。
            if guard.set.len() >= MAX_NONCE_CACHE_ENTRIES {
                tracing::warn!(
                    "nonce 缓存已达硬上限 {}，强制驱逐最旧条目",
                    MAX_NONCE_CACHE_ENTRIES
                );
                // 找到时间戳最小的条目并驱逐（最接近过期）
                if let Some(&(oldest_ts, _)) = guard
                    .set
                    .iter()
                    .min_by_key(|(ts, _)| u64::from_be_bytes(*ts))
                {
                    // 驱逐所有与最旧时间戳相同的条目（批量驱逐提高效率）
                    guard.set.retain(|(ts, _)| *ts != oldest_ts);
                }
            }
