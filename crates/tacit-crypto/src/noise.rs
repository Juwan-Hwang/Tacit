...
impl NonceCache {
    ...
    fn verify_replay(&self, payload: &[u8]) -> CoreResult<()> {
        ...
        // #13 + #15: 时钟回拨检测、prune、nonce 唯一性检查——全部在单一锁区间内完成。
        {
            ...
            // 此警告仅用于诊断，不影响接受/拒绝逻辑。
            //
            // 必须先检查回拨告警，再执行 reset——否则当回拨 > 120s 时，
            // reset 条件（max_seen > now + 60s）先触发，将 max_seen 重置为 now，
            // 导致告警条件（max_seen - timestamp > 120s）永远不满足。

            // 1. 先检查回拨告警（使用 reset 前的 max_seen_timestamp）
            ...
            // 2. 防止日志洪泛：如果 max_seen_timestamp 远超当前系统时间（NTP 回拨），
            //    将其重置为 now，避免后续每次握手都触发 warn 日志。
            if guard.max_seen_timestamp > now + REPLAY_WINDOW_SECS {
                guard.max_seen_timestamp = now;
            ...
            // 硬上限检查——缓存满时批量驱逐 10% 最旧条目，防止 CPU DoS
            //
            // 安全权衡说明：
            // - 原策略（拒绝新条目）：攻击者可通过 flood 100K dummy 握手填满缓存，
            //   阻断所有合法新连接——极易利用的 DoS。
            // - 批量驱逐策略：缓存满时一次性驱逐 10% 最旧条目（约 10K 条），
            //   分摊驱逐成本——驱逐一次后可容纳约 10K 个新握手，不再每次握手都触发 O(N) 扫描。
            // - 驱逐的旧条目已接近过期窗口边缘（60s），安全影响最小。
            // - 内存上限仍由 MAX_NONCE_CACHE_ENTRIES 限制，不会无限增长。
            if guard.set.len() >= MAX_NONCE_CACHE_ENTRIES {
                tracing::warn!(
                    "nonce 缓存已达硬上限 {}，批量驱逐 10% 最旧条目",
                    MAX_NONCE_CACHE_ENTRIES
                );
                // 收集所有条目，按时间戳排序，精确移除最旧的 10%
                //
                // 注意：不能用 threshold + retain 的方式——如果攻击者用相同时间戳 flood，
                // threshold 等于该时间戳，retain(ts > threshold) 会清空整个缓存，
                // 绕过重放保护。必须精确移除 target_index 个条目。
                let mut entries: Vec<_> = guard.set.iter().copied().collect();
                let target_index = entries.len() / 10;
                if target_index > 0 {
                    entries.select_nth_unstable_by_key(target_index, |(ts, _)| {
                        u64::from_be_bytes(*ts)
                    });
                    for entry in entries.iter().take(target_index) {
                        guard.set.remove(entry);
                    }
                }
            }
