...
pub struct NoiseHandshake {
    ...
    local_identity: Option<([u8; 32], [u8; 64])>,
...
impl NoiseHandshake {
        ...
        match self.role {
            ...
            NoiseRole::Responder => match self.step {
                0 => {
                    ...
                    // 提取 replay payload 和 initiator 身份绑定
    ...
    fn build_initiator_payload(&self) -> Vec<u8> {
        ...
        if let Some((pubkey, proof)) = &self.local_identity {
    ...
    fn extract_remote_identity(&mut self, payload: &[u8]) {
    ...
    /// 返回 replay payload 的长度。
    fn extract_replay_and_remote_identity(&mut self, payload: &[u8]) -> usize {
    ...
    pub fn into_transport(self) -> CoreResult<HandshakeResult> {
        ...
        if let Some((ed25519_pubkey, binding_proof)) = &self.remote_identity {
