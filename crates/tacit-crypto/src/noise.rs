...
impl NonceCache {
    ...
    fn record_seen(&self, timestamp: [u8; 8], nonce: [u8; 16]) -> bool {
        ...
        guard.set.insert((timestamp, nonce))
    }
    ...
    fn verify_replay(&self, payload: &[u8]) -> CoreResult<()> {
        ...
        // nonce 唯一性检查（精确重放检测）
        if !self.record_seen(ts_bytes, nonce) {
...
mod tests {
    ...
    fn nonce_cache_independent_across_instances() {
        ...
        assert!(cache2.verify_replay(&payload).is_ok(), "不同 NonceCache 实例应独立");
    }
}
