...
/// 已见 nonce 缓存的最大容量，超过后清空（避免无限增长）。
const SEEN_NONCES_CAP: usize = 1024;
...
/// 当缓存超过 `SEEN_NONCES_CAP` 时清空（滑动窗口策略）。
...
fn record_seen_nonce(timestamp: [u8; 8], nonce: [u8; 16]) -> bool {
    ...
    if set.len() >= SEEN_NONCES_CAP {
        // 缓存满，清空（滑动窗口：旧条目自然过期，因为时间戳验证会拒绝旧消息）
        set.clear();
    }
