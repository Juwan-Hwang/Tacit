    ...
    fn verify_replay(&self, payload: &[u8]) -> CoreResult<()> {
        ...
        // #13 + #15: 时钟回拨检测、prune、nonce 唯一性检查——全部在单一锁区间内完成。
        {
            ...
            if guard.set.len() >= MAX_NONCE_CACHE_ENTRIES {
                ...
                // 收集所有时间戳，用 select_nth_unstable 找 10th percentile（O(N) 而非 O(N log N)）
                let mut timestamps: Vec<u64> = guard
                    .set
                    .iter()
                    .map(|(ts, _)| u64::from_be_bytes(*ts))
                    .collect();
                let target_index = timestamps.len() / 10;
                if target_index > 0 {
                    timestamps.select_nth_unstable(target_index);
                    let threshold = timestamps[target_index];
                    guard
                        .set
                        .retain(|(ts, _)| u64::from_be_bytes(*ts) > threshold);
                }
            }
