...
pub fn serialize_identity(identity: &crate::DeviceIdentity) -> zeroize::Zeroizing<[u8; 160]> {
    ...
    // signing_key_bytes() 和 .private 按值返回 [u8; 32]（Copy 类型），
    // 直接 & 引用会在栈上留下未清零的临时副本。用 Zeroizing 包装确
    // 保临时副本离开作用域时被安全擦除。
    let signing_key = zeroize::Zeroizing::new(identity.signing_key_bytes());
    let static_private = zeroize::Zeroizing::new(identity.static_keypair().private);
    arr[0..32].copy_from_slice(&*signing_key);
    arr[32..64].copy_from_slice(&*static_private);
