...
pub trait SecretStorage: Send + Sync {
    ...
    /// 读取密钥。不存在时返回 `Ok(None)`。
    ///
    /// 返回 `Zeroizing<Vec<u8>>` 确保敏感密钥材料在使用后被安全擦除。
...
impl SecretStorage for KeyringStorage {
    ...
    fn load_secret(&self, key: &str) -> CoreResult<Option<zeroize::Zeroizing<Vec<u8>>>> {
        ...
        match entry.get_secret() {
            Ok(data) => Ok(Some(zeroize::Zeroizing::new(data))),
...
pub fn serialize_identity(identity: &crate::DeviceIdentity) -> zeroize::Zeroizing<[u8; 160]> {
    ...
    // signing_key_bytes() 按值返回 [u8; 32]（Copy 类型）。
    // 用 Zeroizing 包装确保至少 Zeroizing 内部的副本在 drop 时被清零。
    // 原始栈临时变量虽然仍会残留（Copy 的固有限制），但 Zeroizing 能
    // 清零其持有的副本，比不包装更安全。
    // static_keypair().private 是引用字段，copy_from_slice 不产生临时副本。
    let signing_key = zeroize::Zeroizing::new(identity.signing_key_bytes());
    arr[0..32].copy_from_slice(&*signing_key);
...
pub fn deserialize_identity(
    ...
    let mut static_public = zeroize::Zeroizing::new([0u8; 32]);
    static_public.copy_from_slice(&bytes[64..96]);
    let mut binding_proof = zeroize::Zeroizing::new([0u8; 64]);
    binding_proof.copy_from_slice(&bytes[96..160]);

    // 验证 X25519 密钥对一致性：从私钥推导公钥，与存储的公钥比较。
    // 防止存储损坏或篡改导致私钥/公钥不匹配。
    {
        use curve25519_dalek::montgomery::MontgomeryPoint;
        let derived = MontgomeryPoint::mul_base_clamped(*static_private).to_bytes();
        if derived != *static_public {
            return Err(tacit_core::CoreError::Crypto(
                "X25519 密钥对不一致：私钥推导的公钥与存储的公钥不匹配".to_string(),
            ));
        }
    }
    ...
    // signing_key_bytes (Zeroizing<[u8;32]>) 离开作用域，自动零化
    // static_private (Zeroizing<[u8;32]>) 离开作用域，自动零化
    // static_public (Zeroizing<[u8;32]>) 离开作用域，自动零化
    // binding_proof (Zeroizing<[u8;64]>) 离开作用域，自动零化
}
...
mod tests {
    ...
    fn identity_storage_roundtrip() {
        ...
        let loaded = storage
            ...
            .unwrap()
            .unwrap();
        let restored = deserialize_identity(loaded).unwrap();
