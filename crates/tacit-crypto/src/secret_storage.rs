...
/// 共 160 字节。
///
/// 返回 `Zeroizing<Vec<u8>>` 确保堆内存在 drop 时被安全擦除。
pub fn serialize_identity(identity: &crate::DeviceIdentity) -> zeroize::Zeroizing<Vec<u8>> {
...
pub fn deserialize_identity(
    ...
    // 使用栈分配的 [u8;32] 而非堆分配的 Vec，避免堆内存残留
    let signing_key_bytes = zeroize::Zeroizing::new({
        let mut arr = [0u8; 32];
