...
/// 共 160 字节。
pub fn serialize_identity(identity: &crate::DeviceIdentity) -> Vec<u8> {
    let mut buf = Vec::with_capacity(160);
    buf.extend_from_slice(&identity.signing_key_bytes());
    buf.extend_from_slice(&identity.static_keypair().private);
    buf.extend_from_slice(&identity.static_keypair().public);
    buf.extend_from_slice(identity.binding_proof());
    buf
}
...
pub fn deserialize_identity(bytes: &[u8]) -> CoreResult<crate::DeviceIdentity> {
    if bytes.len() != 160 {
    ...
    // 使用 Zeroizing 包装敏感私钥材料，确保离开作用域时安全擦除
    let signing_key_bytes = zeroize::Zeroizing::new(bytes[..32].to_vec());
    let static_private = zeroize::Zeroizing::new({
        let mut arr = [0u8; 32];
    ...
    let result = crate::DeviceIdentity::from_keys(&signing_key_bytes, static_kp, &binding_proof);

    // static_private 被 Zeroizing 包装，离开作用域时自动零化
    // signing_key_bytes 同理
    result
}
...
mod tests {
    ...
    fn identity_serialize_deserialize_roundtrip() {
        ...
        let deserialized = deserialize_identity(&serialized).unwrap();
    ...
    fn deserialize_wrong_length_errors() {
        let result = deserialize_identity(&[0u8; 100]);
