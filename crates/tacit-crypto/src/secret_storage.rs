...
pub fn serialize_identity(identity: &crate::DeviceIdentity) -> zeroize::Zeroizing<[u8; 160]> {
    ...
    // 直接从引用 copy_from_slice 到 Zeroizing 数组内部，
    // 不创建中间 Zeroizing 包装的临时变量（Copy 类型无法被
    // Zeroizing::new 有效保护——按值传递仍会触发栈复制）。
    // 对于 static_keypair().private（引用字段），完全不产生临时副本。
    // 对于 signing_key_bytes()（按值返回），临时返回值在 copy 后立即
    // 被释放，无法被 Zeroizing 追踪——这是函数签名的固有限制。
