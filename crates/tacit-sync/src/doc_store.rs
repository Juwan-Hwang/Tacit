...
impl DocStore {
    ...
    pub fn import_blocks_batch(
        ...
        let rollback_memory = |changed: &std::collections::HashMap<
            BlockId,
            (Vec<u8>, BlockKind, Vec<u8>),
        >| {
            for (block_id, (_, kind, old_snap)) in changed {
                let restored = if !old_snap.is_empty() {
                    BlockDoc::from_snapshot(block_id.clone(), *kind, &self.peer_id, old_snap)
                } else {
                    BlockDoc::new(block_id.clone(), *kind, &self.peer_id)
                };
        ...
        // 内存导入阶段：若中途任一 block 导入失败，回滚已变更的内存状态
        for (block_id, bytes) in blocks {
            ...
            let result = match block.import(bytes) {
                ...
                Err(e) => {
                    ...
                    rollback_memory(&changed_blocks);
                    return Err(e);
                }
            };
            if result.changed {
