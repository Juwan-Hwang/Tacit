...
impl DocStore {
    ...
    pub fn import_blocks_batch(
        ...
        // rollback_list 记录所有「尝试过导入」的 block_id。
        // 回滚时直接清除缓存条目，无需保存旧 snapshot。
        let mut rollback_list: Vec<BlockId> = Vec::new();

        // 闭包：回滚已变更的 block 内存状态
        // 事务已回滚，DB 仍保持正确的历史状态，直接清除缓存条目即可，
        // 下次访问时从 DB 重新加载。
        let rollback_memory = |targets: &[BlockId]| {
            for block_id in targets {
                self.cache.remove(block_id);
                tracing::debug!(block_id = %block_id, "已清除缓存以回滚内存状态");
            }
        };
        ...
        for (block_id, bytes) in blocks {
            ...
            // 在 import 之前就记录，确保即使 import 失败也能回滚
            rollback_list.push(block_id.clone());
    ...
    pub fn spawn_cold_gc(
        ...
        tokio::spawn(async move {
            loop {
                ...
                // 使用弱引用：当 DocStore 的所有强引用释放后，upgrade 失败，退出循环。
                // 避免后台任务无限持有 Arc 导致内存泄漏。
                if let Some(this) = weak_this.upgrade() {
                    ...
                    debug!("DocStore 已释放，停止冷 block GC 后台任务");
                    break;
                }
            }
        })
