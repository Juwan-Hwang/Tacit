...
struct PeerSyncState {
    ...
    ///
    /// 更新时机：
    /// - `apply_remote_block_delta`：收到 peer 发来的 block 数据后，设为本地 apply 后的 frontier。
    /// - `run_push_pull`：向 peer 发送 block 数据后，乐观设为本地当前 frontier。
    ///
    /// 冷启动 / 首次同步时为空 → `run_push_pull` 走 snapshot 路径（同旧行为）。
    /// 乐观更新可能因发送失败而过时，但下次 `export_block_delta` 会失败并回退 snapshot，
    /// 不影响正确性。
...
impl DefaultSyncEngine {
    ...
    fn run_push_pull(&self, peer_id: &PeerId, doc_id: &DocId) -> CoreResult<()> {
        ...
        for block in blocks {
            ...
            // 乐观更新：假设对端成功接收后持有本地当前 frontier。
            // 若发送失败，下次 delta 导出会失败并回退 snapshot，不影响正确性。
            {
                let mut states = self.peer_states.lock();
                if let Some(s) = states.get_mut(peer_id) {
                    s.block_frontiers.insert(
                        (doc_id.clone(), block.block_id.clone()),
                        local_block_frontier.clone(),
                    );
                }
            }
            self.push_action(SyncAction::RequestDelta {
