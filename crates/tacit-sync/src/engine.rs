...
impl DefaultSyncEngine {
    ...
    fn run_push_pull(&self, peer_id: &PeerId, doc_id: &DocId) -> CoreResult<()> {
        ...
        for block in blocks {
            ...
            // peer_bf 只含远端条目（不含本地 peer 的），所以不能直接 covers(local_block_frontier)。
            // 改为：只检查 local_block_frontier 中非本地 peer 的条目是否被 peer_bf 覆盖。
            // 本地条目的覆盖通过 peer 回传的 delta 间接确认（若 peer 已收到本地编辑，
            // 其回传的 delta baseline 会包含本地 peer 的 seq）。
            if !peer_bf.is_empty() {
                ...
                if !non_local_frontier.is_empty() && peer_bf.covers(&non_local_frontier) {
                    continue;
                }
            }
