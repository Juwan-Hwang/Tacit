                    ...
                    if !records.is_empty() {
                        ...
                        for rec in records.iter().rev() {
                            ...
                            let bytes_res = if let Some(ref bid) = block_id {
                                // 优先用对端上报的 summary.frontier 做增量导出，
                                // 减少带宽开销；空 frontier 或 delta 失败时回退到 snapshot
                                if summary.frontier.is_empty() {
                                    self.doc_store.export_block_snapshot(&rec.doc_id, bid)
                                } else {
                                    self.doc_store
                                        .export_block_delta(&rec.doc_id, bid, &summary.frontier)
                                        .or_else(|_| {
                                            self.doc_store.export_block_snapshot(&rec.doc_id, bid)
                                        })
                                }
