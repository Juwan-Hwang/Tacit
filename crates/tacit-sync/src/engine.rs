...
struct PeerSyncState {
    ...
    /// 是否在线。
    online: bool,
    /// per-block 已知 frontier：我们已知该 peer 持有的每个 block 的最新 frontier。
    ///
    /// 仅在 `apply_remote_block_delta`（收到 peer 发来的 block 数据）时更新，
    /// 确保只在传输确认成功后才走 delta 路径。
    ///
    /// 冷启动 / 首次同步时为空 → `run_push_pull` 走 snapshot 路径（同旧行为）。
    /// 不做乐观更新——乐观更新在发送失败时会导致 `export_block_delta` 返回空 delta
    /// （Ok 而非 Err），snapshot 回退不触发，peer 永久缺失基座数据。
    block_frontiers: std::collections::HashMap<(DocId, BlockId), Frontier>,
}
...
impl DefaultSyncEngine {
                    ...
                    if peer.peer_id != *my_id && peer.trust_state == tacit_core::TrustState::Trusted
                    {
                        map.insert(
                            ...
                            PeerSyncState {
                                ...
                                online: false,
                                block_frontiers: std::collections::HashMap::new(),
                            },
                        );
                    }
                }
    ...
    ///
    /// 宿主在完成 Noise 握手 / SAS 配对后调用此方法，将 peer 从 Pending 升级为 Trusted。
    /// 同时清除 `untrusted_cache` 中的负缓存条目（否则 `on_peer_summary` 会因
    /// 负缓存命中而永久拒绝该 peer），并初始化 `peer_states` 条目。
    pub fn upgrade_peer_trust(&self, peer_id: &PeerId, device_pubkey: &str) -> CoreResult<()> {
        // 1. 更新 DB 中的 trust_state
        let conn = self.doc_store.store().conn();
        let existing = tacit_store::dao::get_peer(&conn, peer_id)?;
        match existing {
            Some(mut record) => {
                record.trust_state = tacit_core::TrustState::Trusted;
                if !device_pubkey.is_empty() {
                    record.device_pubkey = device_pubkey.to_string();
                } else if record.device_pubkey.is_empty() {
                    return Err(tacit_core::CoreError::Sync(format!(
                        "upgrade_peer_trust: peer {} 已在 DB 中但 device_pubkey 为空且未提供新值",
                        peer_id
                    )));
                }
            ...
            None => {
                // peer 不在 DB 中（如通过 Noise 握手直接信任的新 peer），
                // 创建新 PeerRecord 并标记为 Trusted。
                if device_pubkey.is_empty() {
                    return Err(tacit_core::CoreError::Sync(format!(
                        "upgrade_peer_trust: peer {} 不在 DB 中且未提供 device_pubkey",
                        peer_id
                    )));
                }
        ...
        tracing::info!(peer = %peer_id, "peer 信任状态已升级为 Trusted");
        Ok(())
    }

    /// 标记 peer 为离线状态。
    ///
    /// 当传输层报告 `PeerOffline` 时调用，使 `push_local_change` 能正确
    /// 将后续变更写入 store-and-forward 而非直接 SendData（避免数据丢失）。
    pub fn mark_peer_offline(&self, peer_id: &PeerId) {
        let mut states = self.peer_states.lock();
    ...
    pub fn apply_remote_block_delta(
        ...
        // 在导入前捕获 frontier，用于计算远端 delta 实际贡献了哪些条目。
        let frontier_before = self.doc_store.block_frontier(doc_id, block_id)?;
        ...
        if result.changed {
            ...
            tacit_store::dao::upsert_ack(
                ...
                &AckSummary {
                    ...
                    doc_id: doc_id.clone(),
                    ack_checkpoint: None,
                    ack_frontier: frontier.clone(),
                    updated_at: SystemTime::now(),
                    version_override: None,
                },
            )?;
            // 更新 per-peer block_frontiers：只记录远端 delta 实际新增/更新的条目
            // （导入前后的差异），并排除本地 peer 的条目。
            // 如果直接记录本地 post-import frontier，会包含本地未发送的编辑，
            // 导致 run_push_pull 的 export_block_delta(since=frontier) 跳过这些编辑。
            let local_pid = self.peer_id().0.clone();
            let remote_frontier = Frontier::from_iter(
                frontier
                    .entries()
                    .filter(|(peer, _)| *peer != local_pid.as_str())
                    .filter(|(peer, seq)| {
                        // 只保留 before 中不存在或 seq 更大的条目（即 delta 新增的）
                        frontier_before
                            .get(&PeerId::new(*peer))
                            .is_none_or(|old| *seq > old)
                    })
                    .map(|(peer, seq)| (PeerId::new(peer), seq)),
            );
            if !remote_frontier.is_empty() {
                let mut states = self.peer_states.lock();
                if let Some(s) = states.get_mut(peer_id) {
                    // 合并而非替换：新 delta 只包含远端新增的条目，
                    // 之前已知的条目（如来自前次 snapshot 的）必须保留。
                    // 用 insert 会丢失旧条目，导致 coverage gate 低估对端知识，
                    // 引起冗余推送。
                    let key = (doc_id.clone(), block_id.clone());
                    s.block_frontiers
                        .entry(key)
                        .and_modify(|existing| {
                            existing.merge(&remote_frontier);
                        })
                        .or_insert(remote_frontier);
                }
            }
        }
        Ok(())
    ...
    fn push_local_change(&self, doc_id: &DocId, change: &ChangeEnvelope) -> CoreResult<()> {
        ...
        if !offline_peer_ids.is_empty() {
            ...
            for peer_id in &offline_peer_ids {
                // 使用 \x1F (ASCII Unit Separator) 作为分隔符，避免 doc_id/block_id
                // 含 ':' 时 entry_id 碰撞导致 sync_log INSERT OR REPLACE 静默覆盖。
                let sep = '\x1f';
                let (entry_id, delta_id) = if let Some(block_id) = &change.block_id {
                    (
                        format!("{peer_id}{sep}{doc_id}{sep}{block_id}{sep}{frontier_str}"),
                        format!("block_delta{sep}{block_id}"),
                    )
                } else {
                    (
                        format!("{peer_id}{sep}{doc_id}{sep}meta{sep}{frontier_str}"),
                ...
                {
                    // 编辑已通过 apply_local_edit 持久化，传播错误会让 UI 误以为编辑失败而重试，
                    // 导致 double-apply。改为 error! 级别日志：变更已持久化，store-and-forward
                    // 记录失败时，对端上线后 on_peer_summary → run_push_pull 会导出当前状态（含此编辑）。
                    error!(
                        peer_id = %peer_id,
                        doc_id = %doc_id,
                        error = %e,
                        "store-and-forward 记录待发失败；变更已持久化，对端上线后通过 push/pull 补发"
                    );
                }
            }
            debug!(
    ...
    /// 处理对端 AckSummary：持久化 ack_frontier 到 DB，并更新内存 block_frontiers。
    ///
    ...
    /// # block_frontiers 更新
    ///
    /// ack_frontier 是 peer 确认已收到的最新 frontier。对于本地 authored block，
    /// `block_frontiers` 之前不会被更新（它只在 `apply_remote_block_delta` 时更新），
    /// 导致后续 `push_local_change` 始终走 snapshot 路径（全量发送）。
    ///
    /// 此处在收到 peer 的 ack 后，将 ack_frontier 中的本地 peer 条目合并到
    /// 对应 block 的 `block_frontiers` 中，使后续编辑走 delta 路径。
    pub fn handle_ack_summary(&self, msg: &tacit_core::AckSummary) -> CoreResult<()> {
        let conn = self.doc_store.store().conn();
        tacit_store::dao::upsert_ack(&conn, msg)?;

        // 将 ack_frontier 中的本地 peer 条目合并到 block_frontiers。
        //
        // 安全约束：ack_frontier 是 per-doc 聚合的（不区分 block），
        // 可能包含 peer 尚未实际收到数据的 block 的 frontier 条目。
        // 因此只更新本地 peer 条目中 seq <= block 实际 frontier 的部分——
        // 即只确认 peer 收到的编辑量不超过 block 当前状态，避免过度推进。
        //
        // 即使此处过度推进，push_local_change 中的空 delta 回退也会防护。
        let local_pid = self.peer_id().0.as_str();
        let local_ack_seq = msg.ack_frontier.get(&PeerId::new(local_pid));
        if let Some(ack_seq) = local_ack_seq {
            if let Ok(blocks) = self.doc_store.list_active_blocks(&msg.doc_id) {
                let mut states = self.peer_states.lock();
                if let Some(s) = states.get_mut(&msg.peer_id) {
                    for block in &blocks {
                        // 获取该 block 的当前 frontier，取本地 peer 的 seq
                        if let Ok(block_frontier) =
                            self.doc_store.block_frontier(&msg.doc_id, &block.block_id)
                        {
                            // 只合并 ack_seq 中 <= block 实际 frontier 的部分
                            let block_local_seq = block_frontier.get(&PeerId::new(local_pid));
                            if let Some(block_seq) = block_local_seq {
                                let safe_seq = ack_seq.min(block_seq);
                                let safe_entries =
                                    Frontier::from_iter([(PeerId::new(local_pid), safe_seq)]);
                                let key = (msg.doc_id.clone(), block.block_id.clone());
                                s.block_frontiers
                                    .entry(key)
                                    .and_modify(|existing| existing.merge(&safe_entries))
                                    .or_insert(safe_entries);
                            }
                        }
                    }
                }
    ...
    fn run_push_pull(&self, peer_id: &PeerId, doc_id: &DocId) -> CoreResult<()> {
        ...
        let (peer_frontier, peer_block_frontiers) = {
            let peer_state = self.peer_states.lock();
            peer_state
                .get(peer_id)
                .map(|s| (s.known_frontier.clone(), s.block_frontiers.clone()))
                .unwrap_or_default()
        };
        ...
        for block in blocks {
            ...
            // 增量 block 同步：若已知 peer 的 block frontier（非空），则导出 delta
            // （仅发送对端缺失的部分）；否则回退到完整 snapshot。
            // delta 导出失败时也回退到 snapshot，保证正确性。
            let peer_bf = peer_block_frontiers
                .get(&(doc_id.clone(), block.block_id.clone()))
                .cloned()
                .unwrap_or_default();

            // 覆盖门控：检查 peer 是否已持有此 block 的全部已知条目。
            // peer_bf 只含远端条目（不含本地 peer 的），所以：
            // 1. 非本地条目：直接检查 peer_bf.covers(non_local_frontier)
            // 2. 本地条目：peer_bf 不包含本地条目（设计决策），无法确认 peer 是否已收到。
            //    如果 local_block_frontier 有本地条目（即我们编辑过此 block），
            //    必须推送——peer 可能还没有我们的编辑。
            // 注意：不能 skip RequestDelta pull——即使 peer 已覆盖非本地条目，
            // peer 可能有本地缺失的更新需要拉取。
            let skip_push = if !peer_bf.is_empty() {
                let local_pid = self.peer_id().0.as_str();
                let has_local_entries = local_block_frontier
                    .entries()
                    .any(|(peer, _)| peer == local_pid);
                if has_local_entries {
                    // 我们有本地编辑，但 peer_bf 不含本地条目，
                    // 无法确认 peer 是否已收到——必须推送。
                    false
                } else {
                    let non_local_frontier = Frontier::from_iter(
                        local_block_frontier
                            .entries()
                            .filter(|(peer, _)| *peer != local_pid)
                            .map(|(peer, seq)| (PeerId::new(peer), seq)),
                    );
                    !non_local_frontier.is_empty() && peer_bf.covers(&non_local_frontier)
                }
            } else {
                false
            };
            ...
            if !skip_push {
                let delta = if peer_bf.is_empty() {
                    self.doc_store
                        .export_block_snapshot(doc_id, &block.block_id)?
                } else {
                    self.doc_store
                        .export_block_delta(doc_id, &block.block_id, &peer_bf)
                        .or_else(|_| {
                            self.doc_store
                                .export_block_snapshot(doc_id, &block.block_id)
                        })?
                ...
                // 不在此处乐观更新 block_frontiers。
                // block_frontiers 仅在 apply_remote_block_delta（收到对端确认数据）时更新，
                // 确保只在传输成功后才走 delta 路径。乐观更新会导致发送失败后
                // export_block_delta 返回空 delta（Ok 而非 Err），snapshot 回退不触发，
                // peer 永久缺失基座数据。
            }
            // 始终拉取 peer 的更新（即使我们跳过了推送）。
            self.push_action(SyncAction::RequestDelta {
                peer_id: peer_id.clone(),
                doc_id: doc_id.clone(),
                block_id: Some(block.block_id.clone()),
                since: local_block_frontier,
                priority: Priority::High,
            });
        }
        Ok(())
    ...
    /// 标记已信任 peer 为在线状态（不修改 frontier）。
    ///
    /// 供 FFI 层的 `ffi_on_peer_online` 调用——当宿主只调 `ffi_on_peer_online`
    /// 而尚未调 `ffi_on_peer_summary` 时，确保 `push_local_change` 路由到
    /// live send 而非 store-and-forward。后续 `ffi_on_peer_summary` 会用
    /// 真实 frontier 覆盖并触发 sync。
    ///
    /// **信任验证**：若 peer 不在 `peer_states` 中（即未在 engine 构造时预加载），
    /// 会查询 DB 确认其 `trust_state == Trusted` 后才创建 entry。
    /// 非信任 peer 不会被插入 `peer_states`，维护"peer_states 中的 peer 必然是
    /// Trusted"的不变量。
    ///
    /// 返回 `true` 表示 peer 已标记 online，`false` 表示 peer 未信任（已忽略）。
    pub fn mark_peer_online(&self, peer_id: &PeerId) -> bool {
        // 快路径：peer 已在 peer_states 中
        {
            let mut states = self.peer_states.lock();
            if let Some(s) = states.get_mut(peer_id) {
                s.online = true;
        ...
        // 慢路径：peer 不在 peer_states 中，查 DB 确认信任状态
        // DoS 防护：先查负缓存
        if self.untrusted_cache.lock().contains(peer_id) {
            tracing::warn!(peer = %peer_id, "mark_peer_online 拒绝：负缓存命中");
            return false;
        }
        let is_trusted = {
            let conn = self.doc_store.store().conn();
            match tacit_store::dao::get_peer(&conn, peer_id) {
                ...
                Ok(None) => false,
                Err(e) => {
                    tracing::error!(peer = %peer_id, error = %e, "mark_peer_online 查询信任状态失败");
                    return false;
                }
            }
        };
        if !is_trusted {
            ...
            tracing::warn!(peer = %peer_id, "mark_peer_online 拒绝：peer 未信任");
            return false;
        }
        // 信任通过，创建 entry
        let now = SystemTime::now();
        ...
        states
            ...
            .or_insert_with(|| PeerSyncState {
                ...
                last_seen: now,
                online: true,
                block_frontiers: Default::default(),
            });
        if let Some(s) = states.get_mut(peer_id) {
            s.online = true;
            s.last_seen = now;
...
impl SyncEngine for DefaultSyncEngine {
    ...
    fn on_peer_summary(&self, peer_id: PeerId, summary: PeerSummary) -> CoreResult<()> {
        ...
        if !self.peer_states.lock().contains_key(&peer_id) {
            ...
            let is_trusted = {
                ...
                match tacit_store::dao::get_peer(&conn, &peer_id) {
                    ...
                    Ok(None) => false,
                    Err(e) => {
                        // DB 错误（如锁竞争、I/O 故障）不应写入负缓存，
                        // 否则瞬态错误会变成永久拒绝（直到 engine 重启）。
                        tracing::error!(peer = %peer_id, error = %e, "查询 peer 信任状态失败");
                        return Err(e);
                    }
                }
            };
            if !is_trusted {
                // 缓存所有非 Trusted 的 peer（包括 Pending 和 Revoked），
                // 防止未信任 peer 高频发送 Capabilities 触发重复 DB 查询。
                // 当 Pending peer 通过 handle_introduce 升级为 Trusted 时，
                // handle_introduce 会清除其负缓存条目。
                let mut cache = self.untrusted_cache.lock();
                // 容量限制：防止攻击者通过大量随机 PeerId 耗尽内存。
                // 达到上限时清空整个缓存（安全——负缓存清空只会导致
                // 后续请求重新查一次 DB，不影响正确性）。
                if cache.len() >= 10_000 {
                    cache.clear();
                }
                cache.insert(peer_id.clone());
                return Err(tacit_core::CoreError::Sync(format!(
                    "拒绝未信任 peer {} 的 summary",
                    peer_id
                )));
            }
            // Trusted peer 首次出现，从负缓存中移除。
            // peer_states 的插入由下方 states.insert 统一处理，
            // 无需在此处 or_insert_with（会被下方 insert 覆盖，造成双重锁）。
            self.untrusted_cache.lock().remove(&peer_id);
        }
        ...
        let _was_offline = {
            ...
            // 保留已有的 block_frontiers，仅更新 meta frontier / online / last_seen
            let block_frontiers = states
                .get(&peer_id)
                .map(|s| s.block_frontiers.clone())
                .unwrap_or_default();
            // 若 summary.frontier 为空（如 FFI 宿主不携带 frontier 信息），
            // 保留已有的 known_frontier，避免覆盖 delta 优化基线。
            // FFI 路径（ffi_on_peer_summary）总是传空 frontier，
            // 若始终覆盖会导致每次调用都重置为空，触发全量 snapshot flood。
            // peer 重置场景由 run_push_pull 的 RequestDelta 拉取恢复：
            // peer 收到 RequestDelta 后发现自己无数据，请求 snapshot 即可追上。
            let known_frontier = if summary.frontier.is_empty() {
                states
                    .get(&peer_id)
                    .map(|s| s.known_frontier.clone())
                    .unwrap_or_default()
            } else {
                summary.frontier.clone()
            };
            states.insert(
                peer_id.clone(),
                PeerSyncState {
        ...
        // peer 在线时：重发未投递的待发消息。
        // 不依赖 was_offline 标志——mark_peer_online 可能在 on_peer_summary 之前
        // 已设 online=true，导致 was_offline=false，但 S&F 记录仍需重发。
        if summary.online {
                ...
                Ok(records) => {
                    if !records.is_empty() {
                        ...
                        for rec in records.iter().rev() {
                            // 从 delta_id 解析出 block_id（格式: "block_delta\x1f{id}" 或 "meta_delta"）
                            let block_id = if rec.delta_id == "meta_delta" {
                                None
                            } else if let Some(bid_str) =
                                rec.delta_id.strip_prefix("block_delta\u{1f}")
                            {
                            ...
                            // Block delta：不使用 ack_frontier（可能被 block delta 覆盖），
                            // 直接导出 snapshot。
                            // Meta delta：用 peer 的 known_frontier（来自 Capabilities/PeerSummary）
                            // 作为增量基线，而非 ack_frontier（会被 apply_remote_block_delta
                            // 覆盖为 block frontier，导致 meta delta baseline 不正确）。
                            let peer_meta_frontier = if block_id.is_some() {
                                Frontier::new()
                            } else {
                                let states = self.peer_states.lock();
                                states
                            ...
                            let bytes_res = if let Some(ref bid) = block_id {
                                // Block 的 Frontier 与 MetaDoc 的 summary.frontier 不兼容，
                                // 直接导出 snapshot
                                self.doc_store.export_block_snapshot(&rec.doc_id, bid)
                            } else if peer_meta_frontier.is_empty() {
                                self.doc_store.export_meta_snapshot(&rec.doc_id)
                            } else {
                                self.doc_store
                                    .export_meta_delta(&rec.doc_id, &peer_meta_frontier)
                            ...
                            let bytes = match bytes_res {
                                Ok(b) => b,
                                Err(e) => {
                                    // BlockNotFound/DocNotFound 等永久性错误：
                                    // 标记为已投递，防止无限重试和数据库累积
                                    if matches!(
                                        e,
                                        tacit_core::CoreError::BlockNotFound { .. }
                                            | tacit_core::CoreError::DocNotFound(_)
                                    ) {
                                        dedup_entry_ids.push(rec.entry_id.clone());
                                        continue;
                                    }
                        ...
                        // 实际发送的记录在 SyncSession::drive_outbound 中由
                        // mark_delivered_and_acknowledged_batch 统一标记。
                        // at-most-once 合约：transport Ok = 投递完成（CRDT 幂等保证安全）。
                        // 若帧在传输中丢失，run_push_pull 的 RequestDelta pull 可恢复数据。
                    }
                }
                Err(e) => {
                    debug!(
...
mod tests {
    ...
    /// 测试 (c)：handle_ack_summary 更新 block_frontiers 后，后续编辑走 delta 路径。
    #[test]
    fn ack_summary_enables_delta_path_for_local_block() {
        let (engine, doc_store) = make_engine();
        doc_store
            .create_block(
                &DocId::new("d1"),
        ...
        engine
            ...
            .unwrap();
        let actions = engine.drain_actions();
        let (_, snap_bytes) = extract_send_data(&actions, &pid(2)).expect("第一次应有 SendData");
        assert!(!snap_bytes.is_empty());

        // 2. peer 发回 AckSummary，确认收到本地编辑
        engine
            .handle_ack_summary(&AckSummary {
                peer_id: pid(2),
                doc_id: DocId::new("d1"),
                ack_checkpoint: None,
                ack_frontier: frontier_after_first.clone(),
                updated_at: SystemTime::now(),
                version_override: None,
            })
            .unwrap();

        // 3. 验证 block_frontiers 已更新
        {
            ...
            let bf = s
                ...
                .expect("block_frontier 应已更新");
            assert_eq!(bf.get(&pid(1)), frontier_after_first.get(&pid(1)));
        }

        // 4. 第二次编辑 → 现在应走 delta 路径（block_frontier 非空）
        doc_store
        ...
        let actions = engine.drain_actions();
        let (_, delta_bytes) = extract_send_data(&actions, &pid(2)).expect("第二次应有 SendData");
        assert!(!delta_bytes.is_empty(), "delta 应非空");

        // delta 应小于全量 snapshot
        let full_snap = doc_store
            .export_block_snapshot(&DocId::new("d1"), &BlockId::new("b1"))
            .unwrap();
        assert!(
            delta_bytes.len() <= full_snap.len(),
            "ack 后的 delta ({}) 应 <= snapshot ({})",
            delta_bytes.len(),
            full_snap.len()
        );
    }
