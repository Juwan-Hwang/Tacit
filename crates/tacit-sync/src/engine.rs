...
impl DefaultSyncEngine {
    ...
    pub fn handle_introduce(
        ...
        if msg.introduced_pubkey_hex.len() != 64
            || hex::decode(&msg.introduced_pubkey_hex).is_err()
        {
            return Err(tacit_core::CoreError::Sync(format!(
...
mod tests {
    ...
    fn handle_introduce_rejects_unknown_introducer() {
        ...
        let msg = tacit_transport::IntroducePeer {
            introducer: pid(99),
            introduced_peer: pid(3),
            introduced_pubkey_hex: "deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef".to_string(),
            endpoint: None,
        };
    ...
    fn handle_introduce_does_not_overwrite_existing_peer() {
...

        {
            ...
            tacit_store::dao::upsert_peer(
                ...
                &tacit_core::PeerRecord {
                    peer_id: pid(3),
                    device_pubkey: "aabbccdd11223344aabbccdd11223344aabbccdd11223344aabbccdd11223344".to_string(),
                    capabilities: Default::default(),
                    trust_state: tacit_core::TrustState::Trusted,
                    anchor_priority: 5,
                    last_seen_at: SystemTime::now(),
        ...
        let msg = tacit_transport::IntroducePeer {
            introducer: pid(2),
            introduced_peer: pid(3),
            introduced_pubkey_hex: "1122334455667788112233445566778811223344556677881122334455667788".to_string(),
            endpoint: None,
        };
        engine.handle_introduce(&msg, &pid(2)).unwrap();
        ...
        assert_eq!(peer.device_pubkey, "aabbccdd11223344aabbccdd11223344aabbccdd11223344aabbccdd11223344");
    }
