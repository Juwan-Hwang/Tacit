...
impl DefaultSyncEngine {
    ...
    fn run_push_pull(&self, peer_id: &PeerId, doc_id: &DocId) -> CoreResult<()> {
        ...
        for block in blocks {
            ...
            // Block 和 MetaDoc 是独立的 CRDT 文档，Frontier 不兼容。
            // 无法通过 peer_frontier (MetaDoc) 判断 peer 是否已有 Block 数据。
            // 因此同时执行推送和拉取：
            // - 推送 block snapshot：确保对端能收到它可能没有的 block 数据。
            //   虽然 PendingBlockFetch 也能触发拉取，但在对端已预创建空 block
            //   的场景下 PendingBlockFetch 不会触发，推送是必要的补充。
            // - 拉取 RequestDelta：以 local_block_frontier 为基线请求对端的
            //   block 数据，对端在 handle_need_ranges 中决定返回 delta 或 snapshot。
            let delta = self
                .doc_store
