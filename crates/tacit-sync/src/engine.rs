...
impl DefaultSyncEngine {
        ...
        for (peer_id, meta_since, block_since) in &online_peers {
            let bytes = if let Some(block_id) = &change.block_id {
                if block_since.is_empty() {
                    // peer 首次同步该 block → 发 snapshot（非空 delta 的安全回退）
    ...
    pub fn handle_ack_summary(&self, msg: &tacit_core::AckSummary) -> CoreResult<()> {
        ...
        // ack_frontier 是 peer 确认已收到的全局 frontier，其中的本地 peer 条目
        // 表示 peer 已收到我们的编辑。将其记入 block_frontiers 后，后续
        // push_local_change 会走 delta 路径而非全量 snapshot。
        //
        // 注意：ack_frontier 是 per-doc 聚合的（不含 block_id 信息），
        // 我们需要遍历该 doc 的所有 active block，对每个 block 检查其 frontier
        // 是否被 ack_frontier 覆盖。如果覆盖，说明 peer 已收到该 block 的全部本地编辑。
        let local_pid = self.peer_id().0.as_str();
        let local_entries: Frontier = Frontier::from_iter(
            msg.ack_frontier
                .entries()
        ...
        if !local_entries.is_empty() {
            // 获取该 doc 的所有 active block，逐个更新 block_frontiers
            if let Ok(blocks) = self.doc_store.list_active_blocks(&msg.doc_id) {
                ...
                if let Some(s) = states.get_mut(&msg.peer_id) {
                    for block in &blocks {
                        ...
                        s.block_frontiers
                            .entry(key)
                            .and_modify(|existing| existing.merge(&local_entries))
                            .or_insert_with(|| local_entries.clone());
                    }
                }
            }
        }
