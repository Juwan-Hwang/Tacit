...
impl DefaultSyncEngine {
    ...
    fn run_push_pull(&self, peer_id: &PeerId, doc_id: &DocId) -> CoreResult<()> {
        ...
        for block in blocks {
            ...
            // peer_frontier 是 MetaDoc 的 frontier，不能用于判断 peer 是否
            // 已拥有 Block 数据。因此：
            // - 推送方向：始终推送 block snapshot（无法增量导出）
            // - 拉取方向：始终发送 RequestDelta，以 local_block_frontier
            //   作为 since 基线。对端若不认识该 frontier 则回退到 snapshot。
            let delta = self
                .doc_store
