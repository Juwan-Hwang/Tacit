...
impl DefaultSyncEngine {
    ...
    pub fn apply_remote_block_delta(
        ...
        if result.changed {
            ...
            // 更新 per-peer block_frontiers：记录该 peer 已持有此 block 的当前 frontier。
            // 下次 run_push_pull 据此做增量 delta 导出，避免无条件推送完整 snapshot。
            {
                let mut states = self.peer_states.lock();
                if let Some(s) = states.get_mut(peer_id) {
                    s.block_frontiers
                        .insert((doc_id.clone(), block_id.clone()), frontier);
                }
            }
        }
        Ok(())
