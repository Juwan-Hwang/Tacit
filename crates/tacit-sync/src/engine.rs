    ...
    fn on_peer_summary(&self, peer_id: PeerId, summary: PeerSummary) -> CoreResult<()> {
        ...
        let was_offline = {
            ...
            let block_frontiers = states
                ...
                .unwrap_or_default();
            // 始终用 summary.frontier 覆盖旧状态。
            // 空 frontier 表示 peer 重置了本地存储（如重装/清库）——
            // 保留旧非空 frontier 会导致我们跳过重发，peer 永远无法追上。
            // 多发一次 snapshot 的成本远低于丢失数据的成本。
            let known_frontier = summary.frontier.clone();
            states.insert(
                peer_id.clone(),
                PeerSyncState {
                    known_frontier,
