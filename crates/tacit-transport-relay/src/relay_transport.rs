                        ...
                        tokio::spawn(async move {
                            ...
                            match RelayMessage::from_bytes(&buf) {
                                Ok(msg) => {
                                    ...
                                    let msg = if let RelayMessage::Incoming {
                                        from_peer_id,
                                        data,
                                    } = msg
                                    {
...
mod tests {
    ...
    fn establish_e2e_sessions() -> (Session, Session) {
        ...
        let mut init =
            NoiseHandshake::initiator(id1.static_keypair().private.as_slice()).unwrap();
        let mut resp =
            NoiseHandshake::responder(id2.static_keypair().private.as_slice()).unwrap();
