...
impl RelayClientTransport {
    ...
    fn start_recv_loop(&self, conn: Connection) {
        ...
        tokio::spawn(async move {
            ...
            // 推进条件：收到 seq == watermark + 1（或占位消息）。
            let mut read_watermark: Option<u64> = None;
            let mut pending_seqs: BTreeSet<u64> = BTreeSet::new();

            // Per-peer 重排序缓冲：seq -> ciphertext
            let mut peer_buffers: HashMap<String, BTreeMap<u64, Vec<u8>>> = HashMap::new();
            ...
            while let Some((seq, from_peer_id, data)) = decrypt_rx.recv().await {
                ...
                // ── 处理所有 peer 的可消费消息 ──
                // 条件：seq <= read_watermark（确保所有更小的 seq 均已读取）
                ...
                for (pid, buf) in peer_buffers.iter_mut() {
        ...
        // ── Accept + 并发读取循环 ──
        // 并发读取避免 HoL 阻塞；每条流有 5s 读取超时防止 DoS。
            ...
            loop {
                match conn.accept_uni().await {
                    Ok(mut stream) => {
                        ...
                        tokio::spawn(async move {
                            // 5s 读取超时：防止恶意/故障 relay 保持 stream 不关闭
                            ...
                            match RelayMessage::from_bytes(&buf) {
                                Ok(msg) => {
                                    if let RelayMessage::Incoming { from_peer_id, data } = msg {
                                        let _ = decrypt_tx.send((seq, from_peer_id, data)).await;
                                    } else {
