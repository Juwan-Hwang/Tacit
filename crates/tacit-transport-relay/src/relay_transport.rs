                                    ...
                                    match s.decrypt(&data) {
                                        ...
                                        Err(e) => {
                                            warn!(
                                                peer = %from,
                                                error = %e,
                                                "E2E 解密失败，可能因 rekey 失步，强制 rekey_incoming 并丢弃消息"
                                            );
                                            // 强制 rekey_incoming 追上发送方：
                                            // 发送方可能已 rekey_outgoing 但消息丢失导致
                                            // 接收方 decrypt_count 未达阈值，形成永久失步。
                                            // rekey_incoming 后后续消息可用新密钥解密。
                                            s.rekey_incoming();
                                            continue;
                                        }
                                    }
