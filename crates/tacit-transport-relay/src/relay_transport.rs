                ...
                for (_, msg) in to_process {
                    match msg {
                        RelayMessage::Incoming { from_peer_id, data } => {
                                ...
                                Some(session) => {
                                    ...
                                    match s.decrypt(&data) {
                                        ...
                                        Err(e) => {
                                            ...
                                            //
                                            // 限流策略：rekey_incoming() 重置 decrypt_count=0，
                                            // 解密失败不增加 decrypt_count。所以如果
                                            // decrypt_count==0 说明刚 rekey 过——此时不再
                                            // rekey，而是标记 session 需要移除，强制重连，
                                            // 防止攻击者通过篡改帧触发无限 rekey 推进 HKDF 链。
                            ...
                            // 解密结果：Ok=正常分发，Err(None)=跳过（catch-up rekey），Err(Some)=移除 session
                            let decrypted = match decrypted {
                                Ok(data) => data,
                                Err(None) => continue, // catch-up rekey 后跳过此消息
                                Err(Some(peer_to_remove)) => {
                                    sessions_for_dispatch.write().remove(&peer_to_remove);
                                    continue;
                                }
                            };
