                                ...
                                Some(session) => {
                                    ...
                                    match s.decrypt(&data) {
                                        ...
                                        Err(e) => {
                                            ...
                                            // 只在 incoming_rekey_pending 时 rekey：
                                            // 如果接收方计数未达阈值，说明失步可能是发送方
                                            // 还未 rekey 或帧被篡改——此时不 rekey 避免攻击者
                                            // 通过篡改帧触发无限 rekey DoS。
                                            // 如果计数已达阈值，rekey 是合理的（追上发送方）。
                                            if s.incoming_rekey_pending() {
                                                debug!(
                                                    peer = %from,
                                                    "接收方 rekey 阈值已达，执行 rekey_incoming"
                                                );
                                                s.rekey_incoming();
                                            } else {
                                                debug!(
                                                    peer = %from,
                                                    "接收方未达 rekey 阈值，跳过 rekey（防止 DoS）"
                                                );
                                            }
                                            continue;
                                        }
