...
impl NonceCache {
    ...
    /// 清理过期的 nonce 记录。
    ///
    /// 由于 timestamp + max_age < now 的 proof 会被时间戳检查拒绝，
    /// 所以只要定期清空即可——这里采用简单策略：当缓存超过 10000 条时全量清空。
    /// 这是一个安全 vs 性能的合理折衷：proof 的 max_age 通常只有 60 秒，
    /// 10000 个不同 nonce 足以覆盖 60 秒窗口内的所有合法 proof。
