...
mod tests {
    ...
    #[test]
    fn data_frame_rejects_wrong_version() {
        // 构造一个合法的 v2 帧，然后将 version 字节改为 v1，
        // 验证 decode 拒绝非 PROTOCOL_VERSION 的帧。
        ...
        // 篡改 version 字节（偏移量 2）为 v1
        encoded[2] = 1;
        let result = DataFrameWire::decode(&encoded);
        assert!(result.is_err(), "非 PROTOCOL_VERSION 的帧必须被拒绝");
        match result {
            Err(FrameError::VersionMismatch(1)) => {}
            other => panic!("期望 VersionMismatch(1)，得到 {other:?}"),
        }
    }
