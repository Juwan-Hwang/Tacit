...
impl TacitEngine {
    ...
    pub fn save_device_identity(&self, identity: &DeviceIdentity) -> CoreResult<()> {
        ...
        let rec = tacit_store::dao::DeviceIdentityRecord {
            signing_key: zeroize::Zeroizing::new(identity.signing_key_bytes().to_vec()),
            static_private: zeroize::Zeroizing::new(identity.static_keypair().private.to_vec()),
...
impl TacitEngine {
    ...
    pub fn ffi_generate_and_save_device_identity(
        ...
        if let Some(rec) = tacit_store::dao::load_device_identity(&conn)? {
            ...
            let di = DeviceIdentity::from_keys(&rec.signing_key, static_kp, &rec.binding_proof)?;
        ...
        let rec = tacit_store::dao::DeviceIdentityRecord {
            signing_key: zeroize::Zeroizing::new(di.signing_key_bytes().to_vec()),
            static_private: zeroize::Zeroizing::new(di.static_keypair().private.to_vec()),
            static_public: di.static_keypair().public.to_vec(),
