...
impl TacitEngine {
    ...
    pub fn ffi_on_peer_summary(
        ...
        Ok(self
            .engine
            .on_peer_summary(summary.peer_id.clone(), summary)?)
    }
    ...
    pub fn ffi_mark_peer_offline(
        ...
        self.engine.mark_peer_offline(&PeerId::new(peer_id));
        Ok(())
    }
    ...
    pub fn ffi_upgrade_peer_trust(
        ...
        Ok(self.engine.upgrade_peer_trust(&PeerId::new(peer_id))?)
    }
    ...
    pub fn ffi_mark_delivered(&self, entry_id: String) -> Result<(), crate::error::TacitFfiError> {
        let conn = self.doc_store.store().conn();
        tacit_store::dao::mark_delivered_batch(&conn, &[&entry_id], std::time::SystemTime::now())
