...
impl TacitEngine {
    ...
    pub fn ffi_save_device_identity(
        &self,
        identity: crate::view::FfiDeviceIdentity,
    ) -> Result<(), crate::error::TacitFfiError> {
        let static_kp =
            tacit_crypto::StaticKeypair {
                private: identity.static_private.as_slice().try_into().map_err(|_| {
        ...
        // #16: 使用后立即擦除 FFI 边界中的敏感私钥字节
        use zeroize::Zeroize;
        let mut identity = identity;
        identity.signing_key.zeroize();
        identity.static_private.zeroize();
