...
pub trait ForeignSecretStorage: Send + Sync {
    /// 存储密钥。同名密钥会被覆盖。
    fn store_secret(&self, key: String, value: Vec<u8>) -> Result<(), String>;

    /// 读取密钥。不存在时返回 `Ok(None)`。
    fn load_secret(&self, key: String) -> Result<Option<Vec<u8>>, String>;

    /// 删除密钥。不存在时返回 `Ok(())`。
    fn delete_secret(&self, key: String) -> Result<(), String>;
}
