...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 1. 创建内存引擎。
    //    生产环境中，应先 ensure_device_identity() 获取设备身份的 PeerId，
    //    再用该 PeerId 创建引擎（持久化模式下第二次启动即可正确匹配）。
    //    本示例为简化演示，使用 "1" 作为 PeerId。
    let engine = TacitEngine::new_memory("1")?;
    println!("[1] 引擎已创建（内存模式），PeerId = 1");

    // 2. 自动生成/加载设备身份
    //    生成的身份 PeerId 来自 Ed25519 公钥的 hex，与引擎的 PeerId "1" 不同。
    //    持久化模式下，下次启动时应用身份的 PeerId 创建引擎以确保一致。
    ...
    // 触发一个事件（模拟网络变化，会发布 PeerStatusChanged 到 EventBus）
    engine.ffi_notify_network_changed(true, "lan".into())?;
    println!("    已通知网络变化：online = true, net_type = lan");
